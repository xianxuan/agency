...
<body>
<form action="<%=basePath%>serviceAction!editService.action" method="post"  class="definewidth m20" >  
   <input type="hidden" id="adminId" name="service.serviceId" value="${service.serviceId }"/>
   <input type="hidden" id="AAccount" name="service.STheme" value="${service.STheme}"/></td>         
   <table class="table table-bordered table-hover definewidth m10" style="height:100%;">
              <tr>
        
            <td width="10%" class="tableleft">服务名称</td>
            <td>${service.STheme }</td>
        </tr>
        
        <tr>
            <td class="tableleft">服务内容</td>
            <td ><input type="text" id="AAccount" name="service.SContent" value="${service.SContent }"/></td>
        </tr>
        <tr>
...
$(function () {
	...
	$('#backid').click(function(){
		...
		window.location.href="<%=basePath%>serviceAction.action";
 });
