    ...
    <div class="big">
      ...
      <s:include value="/new/left.jsp" />
        ...
        <div class="right">
           ...
           <div class="search-content" style="margin-top:10px;margin-bottom:10px;margin-left:200px;">
				<form id = "form1"
					action="<%=basePath%>houseAction!getHousesByName.action"
					method="post">
					<label style="margin-left:80px;">房屋名称：</label><input id="num" type="text" class="w150" name="name"
						value="${name }"  style="height:25px;"> 
				  <input type="button" id="TJ" value="查询"style="margin-left:80px;height:25px;width:80px;"> 
               	<a href="javascript:void(0);" onclick="window.location.href='/agency/jsp/House/add.jsp'"style="margin-left:80px;height:25px;width:80px;">添加房屋</a>  					               
				</form>
			</div>
               ...
               <s:iterator value="houses" var="house">
                <tr>
                 <td>
                    ...
                    <td>
                     	<a href="<%=basePath%>houseAction!jumpEditHouse.action?houseId=${house.houseid}"><span class="w40">编辑</span></a> 
                     	<c:if test="${house.isrent == '否' }">
                     	<a href="<%=basePath%>houseAction!rentHouse.action?houseId=${house.houseid}"onclick="return rent()"><span class="w40">出租</span></a>
                     	</c:if>
                     	<c:if test="${house.isrent == '是' }">
                     	<a href="<%=basePath%>houseAction!callHouse.action?houseId=${house.houseid}"onclick="return call()"><span class="w40">招租</span></a>
                     	</c:if>
                     	<a href="<%=basePath%>houseAction!deleteHouse.action?houseId=${house.houseid}"onclick="return del()"><span class="w40">删除</span></a>                  
                    </td>
                </tr>
               </s:iterator>
            </table>
